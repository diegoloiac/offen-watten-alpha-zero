...
import versions.asymmetric_sub_watten.asymmetric_sub_watten as asymmetric_sub_watten
import numpy as np

from core.interfaces.Game import Game


class AsymmetricSubWattenGame(Game):
    def __init__(self):
        super().__init__()
        self.trueboard = asymmetric_sub_watten.WorldAsymmetricSubWatten()
    ...
    def clone(self):
        cloned_game = AsymmetricSubWattenGame()
    ...
    def get_player_visible_state(self, player):
        ...
        last_card_deck = self.trueboard.last_card_deck if player_in == self.trueboard.distributing_cards_player\
            else None
